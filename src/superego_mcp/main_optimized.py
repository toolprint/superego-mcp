...
async def async_main() -> None:
    ...
    if config.ai_sampling.enabled:
        ...
        print(
            f"Request queue configured with max size: {config.performance.request_queue.max_size}"
        )
    ...
    if config.performance.metrics_enabled:
        ...
        print(
            f"Monitoring dashboard available at http://localhost:{config.performance.metrics_port}/dashboard"
        )
        print(
            f"Prometheus metrics available at http://localhost:{config.performance.metrics_port}/metrics"
        )
    ...
    async def collect_metrics() -> None:
        while not shutdown_event.is_set():
            ...
            if alert_manager:
                ...
                for alert in alerts:
                    print(
                        f"ALERT: {alert['name']} - {alert['severity']}: {alert['current_value']} > {alert['threshold']}"
                    )
    ...
    try:
        ...
        print(
            f"- Response caching: TTL={config.performance.caching.response_cache_ttl}s"
        )
        print(
            f"- Connection pooling: max={config.performance.connection_pooling.max_connections}"
        )
        print(
            f"- Request queue: size={config.performance.request_queue.max_size}, concurrency={config.performance.request_queue.ai_sampling_concurrency}"
        )
        if config.performance.batching.enabled:
            print(
                f"- Request batching: size={config.performance.batching.batch_size}, timeout={config.performance.batching.batch_timeout}s"
            )
