...
class OptimizedSecurityPolicyEngine(SecurityPolicyEngine):
    ...
    async def _evaluate_uncached(self, request: ToolRequest, start_time: float) -> Decision:
        ...
        if matching_rule.action == ToolAction.SAMPLE:
            ...
            return await self._handle_sampling_optimized(request, matching_rule, start_time)
