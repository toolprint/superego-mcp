...
class OptimizedClaudeService(BaseAIService):
    ...
    def __init__(self, config: SamplingConfig, api_key: str, connection_pool: ConnectionPool):
...
class OptimizedAIServiceManager(AIServiceManager):
    ...
    async def evaluate_with_ai(self, prompt: str, cache_key: str | None = None) -> AIDecision:
        ...
        if cached:
            ...
            if self.metrics_collector:
                await self.metrics_collector.update_cache_metrics("ai_response", hit=True, size=1)
            return cached
            
        if self.metrics_collector:
            await self.metrics_collector.update_cache_metrics("ai_response", hit=False, size=1)
        ...
        async with self._semaphore:
                ...
                try:
                    ...
                    if self.metrics_collector:
                        await self.metrics_collector.record_ai_sampling(
                            str(self.config.primary_provider),
                            "success"
                        )
            ...
            if self.config.fallback_provider and self.config.fallback_provider in self._services:
                try:
                    ...
                    if self.metrics_collector:
                        await self.metrics_collector.record_ai_sampling(
                            str(self.config.fallback_provider),
                            "success"
                        )
    ...
    def get_health_status(self) -> dict[str, Any]:
        ...
        base_status.update({
            ...
            "queue_stats": self.request_queue.get_stats() if self.request_queue else None,
