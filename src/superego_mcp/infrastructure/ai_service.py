...
class BaseAIService(ABC):
    ...
    def _parse_response(self, response_text: str, provider: AIProvider, model: str, response_time_ms: int) -> AIDecision:
        ...
        try:
            ...
            return AIDecision(
                ...
                reasoning=data.get("reasoning", data.get("reason", "Failed to parse AI response")),
            ...
            self.logger.error("Failed to parse AI response", error=str(e), response=response_text[:200])
...
class ClaudeService(BaseAIService):
    ...
    async def evaluate(self, prompt: str) -> AIDecision:
        ...
        try:
            ...
            self.logger.error("Claude API error", status=e.response.status_code, error=str(e))
            raise SuperegoError(
...
class OpenAIService(BaseAIService):
    ...
    async def evaluate(self, prompt: str) -> AIDecision:
        ...
        try:
            ...
            self.logger.error("OpenAI API error", status=e.response.status_code, error=str(e))
            raise SuperegoError(
                ErrorCode.AI_SERVICE_UNAVAILABLE,
...
class AIServiceManager:
    ...
    async def evaluate_with_ai(self, prompt: str, cache_key: str | None = None) -> AIDecision:
        ...
        async with self._semaphore:
            ...
            if self.config.primary_provider in self._services:
                try:
                    decision = await self._evaluate_with_provider(
                        self._services[self.config.primary_provider],
                        prompt
                    )
            ...
            if self.config.fallback_provider and self.config.fallback_provider in self._services:
                try:
                    decision = await self._evaluate_with_provider(
                        self._services[self.config.fallback_provider],
                        prompt
                    )
    ...
    async def _evaluate_with_provider(self, service: BaseAIService, prompt: str) -> AIDecision:
