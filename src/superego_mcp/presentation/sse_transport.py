...
class SSEManager:
    ...
    async def subscribe(self, event_type: str) -> asyncio.Queue:
        ...
        logger.info("SSE client subscribed", event_type=event_type, total_subscribers=len(self.subscribers[event_type]))
    ...
    def unsubscribe(self, event_type: str, queue: asyncio.Queue) -> None:
        ...
        if event_type in self.subscribers:
            ...
            logger.info("SSE client unsubscribed", event_type=event_type, total_subscribers=len(self.subscribers[event_type]))
...
class SSETransport:
    ...
    async def _create_event_stream(self, event_type: str, request: Request) -> StreamingResponse:
    ...
    async def _create_combined_event_stream(self, request: Request) -> StreamingResponse:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            for task in self.monitoring_tasks:
                task.cancel()
            await self.sse_manager.stop_keepalive()
