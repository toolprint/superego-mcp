...
class TestPatternPerformance:
    ...
    def test_string_matching_performance(self):
        ...
        values = ["exact_match", "different", "another_match", "third_match", "no_match"]
    ...
    def test_regex_compilation_cache_performance(self):
        ...
        test_strings = [
            "test_command", "file.txt", "create_user", "admin_task", 
            "/etc/config.conf", "no_match", "path.txt", "update_record"
        ]
        ...
        assert cached_time < uncached_time * 0.8, "Regex caching not providing expected performance benefit"
    ...
    def test_glob_matching_performance(self):
        ...
        paths = [
            "document.txt", "/etc/config/app.conf", "/app/config/settings.yaml",
            "file5.log", "/home/user/.bashrc", "/var/log/system.log",
            "/etc/passwd", "/home/user/config/app.conf"
        ]
    ...
    def test_jsonpath_compilation_cache_performance(self):
        ...
        print(f"Cache stats: {cache_stats_before['jsonpath_cache']['hits']} hits, "
...
class TestSecurityPolicyPerformance:
    ...
    def setup_method(self):
        ...
        self.mock_ai_manager.evaluate_with_ai = AsyncMock(return_value=self.mock_ai_decision)
        
        self.mock_prompt_builder = MagicMock()
        self.mock_prompt_builder.build_evaluation_prompt = MagicMock(return_value="test prompt")
    ...
    async def test_rule_evaluation_performance(self):
        ...
        assert total_time < 2.0, f"Rule evaluation too slow: {total_time:.3f}s for 300 evaluations"
    ...
    async def test_pattern_cache_effectiveness(self):
        ...
        print(f"First batch: {first_batch_time:.3f}s, Second batch: {second_batch_time:.3f}s")
        ...
        print(f"Regex cache: {cache_stats['regex_cache']['hits']} hits, "
              f"{cache_stats['regex_cache']['misses']} misses")
        print(f"JSONPath cache: {cache_stats['jsonpath_cache']['hits']} hits, "
    ...
    async def test_early_termination_performance(self):
        ...
        operations_per_second = 1000 / total_time
        print(f"Early termination performance: {operations_per_second:,.0f} evaluations/sec")
