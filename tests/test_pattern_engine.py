...
class TestPatternEngine:
    ...
    def test_regex_matching(self):
        ...
        # Complex patterns
        assert self.pattern_engine.match_regex(
            r"^(rm|delete|remove).*", "remove_file"
        )
    ...
    def test_evaluate_condition_tool_name(self):
        ...
        assert not self.pattern_engine._evaluate_condition(condition, self.sample_request)
...
class TestPatternEnginePerformance:
    ...
    def test_regex_compilation_caching(self):
        ...
        assert stats_after["regex_cache"]["misses"] > stats_before["regex_cache"]["misses"]
    ...
    def test_jsonpath_compilation_caching(self):
        ...
        assert stats_after["jsonpath_cache"]["misses"] > stats_before["jsonpath_cache"]["misses"]
        ...
        # Should have one more hit
        assert stats_after["jsonpath_cache"]["hits"] > stats_before["jsonpath_cache"]["hits"]
    ...
    def test_pattern_matching_performance(self):
        ...
        patterns = [
            ...
            {"type": "jsonpath", "pattern": "$[?(@.size > 1000)]"}
        ]
        ...
        assert total_time < 1.0, f"Pattern matching too slow: {total_time:.3f}s for 3000 matches"