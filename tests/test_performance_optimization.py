...
class TestConnectionPool:
    ...
    async def test_connection_reuse(self):
        ...
        assert elapsed < 3.0  # Generous timeout for CI
...
class TestRequestQueue:
    ...
    async def test_queue_backpressure(self):
        ...
        queue = RequestQueue(max_size=2, enable_backpressure=True)
        ...
        with pytest.raises(asyncio.QueueFull):
            queue.queue.put_nowait(MagicMock())
...
async def test_performance_targets():
    ...
    from superego_mcp.domain.security_policy_optimized import OptimizedSecurityPolicyEngine
    from superego_mcp.infrastructure.metrics import MetricsCollector
    from superego_mcp.infrastructure.performance import PerformanceMonitor, ResponseCache
