...
class TestSecurityPolicyPatterns:
    ...
    async def test_jsonpath_pattern_matching(self):
        ...
        mock_prompt_builder.build_evaluation_prompt = MagicMock(return_value="test prompt")
    ...
    async def test_time_based_conditions(self, mock_datetime):
    ...
    async def test_health_check_pattern_stats(self):
        ...
        await self.engine.evaluate(ToolRequest(
            tool_name="rm_test",
            parameters={},
            session_id="test",
            agent_id="test",
        ...
        expected_enabled = sum(1 for rule in self.test_rules["rules"] 
